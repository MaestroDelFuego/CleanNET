...
const folder = './PhishingAuthorityData';

const files = [
  'blockedDomains.json',
  'suspiciousDomains.json',
  'suspiciousKeywords.json',
  'whitelistedDomains.json'
];
...
async function downloadGithubFile(owner, repo, branch, filepath, savePath) {
  ...
  try {
    ...
    console.log(`  => Downloaded and updated local file: ${filepath}`);
  } catch (err) {
...
async function compareAndUpdateFiles() {
  for (const filename of files) {
    ...
    const githubDate = await getGithubFileLastModified(owner, repo, branch, filename);
    ...
    if (!localDate) {
      ...
      await downloadGithubFile(owner, repo, branch, filename, localPath);
      continue;
    }
