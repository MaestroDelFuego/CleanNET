...
const fetch = require('node-fetch'); // For fetching metadata from URLs
...
const owner = 'MaestroDelFuego';
const repo = 'PhishingAuthority-API';
const branch = 'main'; // or 'master'
const localFolder = './PhishingAuthorityData';

const remoteFiles = [
  'PhishingAuthorityData/blockedDomains.json',
  'PhishingAuthorityData/suspiciousDomains.json',
  'PhishingAuthorityData/suspiciousKeywords.json',
  'PhishingAuthorityData/whitelistedDomains.json'
];

// Get last modified date from GitHub commits API
async function getGithubFileLastModified(owner, repo, branch, filepath) {
  ...
  try {
    ...
    console.error(`Error fetching GitHub info for ${filepath}:`, err.message);
    return null;
  }
}

// Get last modified date of local file
async function getLocalFileLastModified(filepath) {
  try {
...
async function downloadGithubFile(owner, repo, branch, filepath, savePath) {
  const rawUrl = `https://raw.githubusercontent.com/${owner}/${repo}/${branch}/${filepath}`;
  try {
    const response = await fetch(rawUrl);
    if (!response.ok) throw new Error(`Failed to download file: ${response.statusText}`);
    const data = await response.text();
    await fs.mkdir(path.dirname(savePath), { recursive: true }); // ensure folder exists
    await fs.writeFile(savePath, data, 'utf8');
    console.log(`  => Downloaded and updated local file: ${savePath}`);
  } catch (err) {
    console.error(`  => Error downloading ${filepath}:`, err.message);
  }
}
async function compareAndUpdateFiles() {
  for (const remotePath of remoteFiles) {
    ...
    const githubDate = await getGithubFileLastModified(owner, repo, branch, remotePath);
    const localDate = await getLocalFileLastModified(localPath);

    if (!githubDate) {
      console.log('  - Could not get GitHub last modified date');
      continue;
    }

    if (!localDate) {
      console.log('  - Local file missing, will download latest from GitHub');
      await downloadGithubFile(owner, repo, branch, remotePath, localPath);
      continue;
    }

    console.log(`  - GitHub last modified: ${githubDate}`);
    console.log(`  - Local last modified: ${localDate}`);

    if (githubDate > localDate) {
      console.log('  => GitHub version is newer. Downloading...');
      await downloadGithubFile(owner, repo, branch, remotePath, localPath);
    } else if (githubDate < localDate) {
      console.log('  => Local version is newer. No action taken.');
    } else {
      console.log('  => Both versions are up-to-date.');
    }
  }
}
...
function version(){
    const versionstring = '0.05';
    const buildDateString  = '12/07/2025';
